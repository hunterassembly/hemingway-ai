# Hemingway Documentation

This is the public documentation hub for `hemingway-ai`.

## Start By Goal

If you are evaluating Hemingway:

- [Quickstart](./getting-started/quickstart.md)
- [Troubleshooting](./troubleshooting.md)

If you are integrating Hemingway into a project:

- [Next.js Setup](./getting-started/nextjs.md)
- [Vite React Setup](./getting-started/vite-react.md)
- [Remix Setup](./getting-started/remix.md)
- [Script-Tag Setup (Any Framework)](./getting-started/script-tag.md)
- [Configuration Reference](./configuration.md)
- [API Reference](./api-reference.md)

For easiest setup in Next.js, use one-process mode via `/api/hemingway/*` route handlers.

If you want to contribute:

- [Contributing Guide](../CONTRIBUTING.md)
- [Code of Conduct](../CODE_OF_CONDUCT.md)
- [Security Policy](../SECURITY.md)

If you need deep implementation details:

- [Agent Reference Docs](./reference/README.md)

## Recommended Reading Order

1. Quickstart
2. Framework-specific setup
3. Configuration and API reference
4. Troubleshooting
